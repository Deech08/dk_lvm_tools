...
class dapMixin(object):
	...
	def get_snr_masked_intensity(self, colname, snr_cut = 3, rayleigh = True, scale = None):
		"""
		...
		if scale is None:
			return np.ma.array(flux, mask = snr < snr_cut|np.isnan(snr))
		elif scale == "log":
			return np.ma.array(np.log(flux.value), mask = snr < snr_cut|np.isnan(snr))
		elif scale == "log10":
			return np.ma.array(np.log10(flux.value), mask = snr < snr_cut|np.isnan(snr))
	...
	def intensity_map(
		...
		**kwargs,
		...
		if smooth:
			...
			if c.__class__ is str:
				zi = griddata((lon_points, lat_points), flux.data.value,
		...
		if colorbar:
			if not "label" in cbar_kwargs:
				if c.__class__ is str:
					cbar_kwargs["label"] = "{} ({})".format(colname, flux.data.unit)
