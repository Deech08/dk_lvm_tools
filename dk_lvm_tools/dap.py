...
class dap(dapMixin, Table):
    ...
    ----------
    parquet_pattern: 'str'
        if provided, uses duckdb SQL queries to live load columsn as they are needed
        loads only coordinates to start
        should be either a single parquet filename or pattern for finding them with glob
    ...
    **kwargs:
    ...
    def __init__(self, 
        ...
        # elif parquet_pattern is not None:
        #   starting_query = """
        #       SELECT
        #           "id",
        #           "ra",
        ...
        #   if parquet_where is not None:
        #       starting_query = f"{starting_query} {parquet_where}"

        #   import duckdb 
        #   from astropy.io.misc.pyarrow.csv import convert_pa_table_to_astropy_table
        #   res = duckdb.query(starting_query)
        #   arrow_res = res.arrow()
        #   t = convert_pa_table_to_astropy_table(arrow_res.read_all())

        #   # Try to get DAP version from filename
        #   v_pattern = r"v_\d+\.\d+\.\d+"
        #   match = re.search(v_pattern, self.parquet_pattern)
        #   if match:
        #       self.dap_ver = match.group().split("v")[-1]
    ...
    def __getitem__(self,item):
        """
        Check if item should be loaded it not already present using parquet query
        """
        if isinstance(item, str) and item not in self.colnames and self.parquet_pattern is not None:
            self._load_data_for_item(item)
            
        return super().__getitem__(item)

    def _load_data_for_item(self,item):
        """
